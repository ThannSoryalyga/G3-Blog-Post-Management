    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        {sponsors.map((sponsor, index) => (
                <div
                  key={index}
                  className="flex items-center justify-center rounded-lg p-4 bg-white shadow-md hover:shadow-xl transition-shadow"
                        >
                    <img
                 src={sponsor.logo}
                  alt={sponsor.name}
                    ...
                    />
                         <h1 className="text-center mt-2 text-sm font-medium">{sponsor.name}</h1>
                      </div>
                       ))}
     
    
                    </div> {/* Closing the sponsor logos grid */}
